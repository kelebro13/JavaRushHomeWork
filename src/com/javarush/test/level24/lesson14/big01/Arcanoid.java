...
public class Arcanoid
{
    ...
    {
        drawBoders(canvas);//отрисуй границы

        for(Brick b : bricks){
            b.draw(canvas);//отрисуй кирпичи
        }
        this.ball.draw(canvas);//отрисуй шарик
        this.stand.draw(canvas);//отрисуй подставку
    }
    ...
    public void move()
    {
        this.ball.move();//двигай шарик
        this.stand.move();//двигай подставку
    }
    ...
    {
        for(Brick b : bricks){
            if(ball.isIntersec(b)){//Тут проверь - столкнулся ли шарик с кирпичем.
                double angel = Math.random() * 360;
                ball.setDirection(angel);
                bricks.remove(b);
                break;//Если да - кирпичь удалить, а шарик запустить в случайно направлении.
