...
public class Solution {
    ...
    public static class AmigoThreadFactory implements ThreadFactory{
        static AtomicInteger numberFactory = new AtomicInteger();
        AtomicInteger numberThreadInFactory = new AtomicInteger();

        public AmigoThreadFactory() {
            numberFactory.getAndIncrement();
        }

        @Override
        public Thread newThread(Runnable r) {
            numberThreadInFactory.getAndIncrement();
            Thread thread = new Thread(r);
            thread.setDaemon(false);
            thread.setPriority(Thread.NORM_PRIORITY);
            thread.setName(String.format("%s-pool-%d-thread-%d", Thread.currentThread().getThreadGroup().getName(), numberFactory.get(), numberThreadInFactory.get()));
            return thread;
        }
    }
}
