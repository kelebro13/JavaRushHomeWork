...
public class Solution {
    ...
    public  <T extends Item> T getProxy(Class<?> sameClass, Class<?> ... interfaces) {
        Class[] mas = new Class[interfaces.length + 1];

        System.arraycopy(interfaces, 0, mas, 0, interfaces.length);
        mas[mas.length - 1] = sameClass;
        return (T) Proxy.newProxyInstance(sameClass.getClassLoader(), mas, new ItemInvocationHandler());
    }
