...
public class Solution {
    ...
    public static Object convertOneToAnother(Object one, Class resultClassObject) throws IOException {
        ...
        String classOne = one.getClass().toString().substring(one.getClass().toString().lastIndexOf("$") + 1).toLowerCase();
        String classTwo = resultClassObject.toString().substring(one.getClass().toString().lastIndexOf("$") + 1).toLowerCase();
        String s = writer.toString().replaceFirst(classOne, classTwo);

        return mapper.readValue(new StringReader(s), resultClassObject);
    }
